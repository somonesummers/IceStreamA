...
roughMbDhDt = Acc(xx,yy)*3.154e7 - imgaussfilt(divFlowMeasures,12);
smoothMbDhDt = Acc(xx,yy)*3.154e7 - imgaussfilt(divFlowMeasures,25);


% ax2 = nexttile(2);
% surf(xx/1e3,yy/1e3,zeros(size(xx)),roughMbDhDt,'edgecolor','none')
% hold on 
% contour(xx/1e3,yy/1e3,sqrt(uu.^2 + vv.^2),[30 30],'k')
% view(2)
% % c = colorbar;
% % c.Label.String = 'Change in Surface Height [m/yr]';
% title('Rough MB dh/dt')
% colormap(ax2,cMapToUse);
% caxis(cValues);
% xlim(xLimits)
% ylim(yLimits)
...
ax3 = nexttile(2);
surf(xx/1e3,yy/1e3,zeros(size(xx)),smoothMbDhDt,'edgecolor','none')
% surf(xx/1e3,yy/1e3,zeros(size(xx)),Acc(xx,yy)*3.154e7 - divFlowMeasures,'edgecolor','none')
hold on 
contour(xx/1e3,yy/1e3,sqrt(uu.^2 + vv.^2),[30 30],'k')
view(2)
...
save('mbDhDt.mat','xx','yy','roughMbDhDt','smoothMbDhDt')