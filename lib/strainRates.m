function [elon, etrans, eshear, Xi, Yi] = strainRates(xy,u,v,h)
%% [elon, etrans, eshear, x, y] = strainRates(xy,u,v,h) 
...
Adapted by Paul Summers 2024 to run on non-geotiff inputs
    %}
    ...
    %Geotiff are upside down, so flip to follow this
    Xi = flipud(Xi);
    Yi = flipud(Yi);
    u_interp = scatteredInterpolant(xy(:,1),xy(:,2),u,'linear','none');
    v_interp = scatteredInterpolant(xy(:,1),xy(:,2),v,'linear','none');
    h_interp = scatteredInterpolant(xy(:,1),xy(:,2),h,'linear','none');

    vx = u_interp(Xi,Yi);
    vy = v_interp(Xi,Yi);
    thick = h_interp(Xi,Yi);
    %  
    ...
    % Set grid parameters
    pixel_size = dx; % Pixel size in distance units 
    ...
    length_scale = 1000; % If not using an ice thickness grid, define a single 
...
%     elonOut   = "Geotiffs/" + file + "elon_r1000";
%     eshearOut = "Geotiffs/" + file + "eshear_r1000";
%     etransOut = "Geotiffs/" + file + "etrans_r1000";
    ...
    elon   = flipud(elon);
    etrans = flipud(etrans);
    eshear = flipud(eshear);
    Xi = flipud(Xi);
    Yi = flipud(Yi);
    ...
    % geotiffwrite(elonOut + ".tif", elon, vx_info, 'GeoKeyDirectoryTag',tiffinfo.GeoTIFFTags.GeoKeyDirectoryTag);
    % geotiffwrite(eshearOut + ".tif", eshear, vx_info, 'GeoKeyDirectoryTag',tiffinfo.GeoTIFFTags.GeoKeyDirectoryTag);
    % geotiffwrite(etransOut + ".tif", etrans, vx_info, 'GeoKeyDirectoryTag',tiffinfo.GeoTIFFTags.GeoKeyDirectoryTag);
    % For saving if you want to save the data
%     save(elonOut + ".mat", 'elon','Xi','Yi','dx');
%     save(eshearOut + ".mat", 'eshear','Xi','Yi','dx');
%     save(etransOut + ".mat", 'etrans','Xi','Yi','dx');
end