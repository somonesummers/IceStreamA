...
% [A] = calcAfromT(T)
% Calculates prefactor A from temp input. From Cuffey & Patterson 3.4.6
% Works for scalar, vector, matrix inputs
    ...
    A = zeros(size(T));
    A(T <= T_star) = E_star*A_star.*exp(-Q1/R.*(1./T(T <= T_star)-1/T_star));
    A(T >  T_star) = E_star*A_star.*exp(-Q2/R.*(1./T(T >  T_star)-1/T_star));
end